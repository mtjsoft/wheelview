...
public class SelectorWheelView extends View
{
	...
	private int cacheNowItem = 0; // 预设当前item的位置，负数表示不设定
	...
	public SelectorWheelView(Context context, AttributeSet attrs)
	{
	...
	private void init()
	{
		...
		paint.setTextSize(HHDensityUtils.dip2px(getContext(), textSize));
	...
	private void measureData()
	{
		if (isStart)
		{
			...
			itemX = width / 2;
			height = getHeight();
			itemHeight = (height - getPaddingTop() - getPaddingBottom())
					/ showSize;
			...
			centerItemTop = (height - getPaddingTop() - getPaddingBottom()) / 2
					+ getPaddingTop() - centerItemHeight / 2;
			centerItemBottom = (height - getPaddingTop() - getPaddingBottom())
					/ 2 + getPaddingTop() + centerItemHeight / 2;
			shader = new LinearGradient(0, 0, 0, height, new int[] {
					0xFFFFFFFF, 0xAAFFFFFF, 0x00FFFFFF, 0x00FFFFFF, 0xAAFFFFFF,
					0xFFFFFFFF }, new float[] { 0.0f, centerItemTop / height,
					centerItemTop / height, centerItemBottom / height,
					centerItemBottom / height, 1.0f }, Shader.TileMode.REPEAT);
	...
	protected void onDraw(Canvas canvas)
	{
		...
		for (int i = startItemPos, j = 0; i < startItemPos + showSize + 2; j++, i++)
		{
			...
			} else
			{
				if (isCircle)
				{
					int pos = i % dataSize;
					canvas.drawText(
							data.get(pos < 0 ? pos + dataSize : pos).showStr,
							itemX, getBaseLine(paint, topY, itemHeight), paint);
				}
			}
		}
		...
		canvas.drawLine(getPaddingLeft(), centerItemTop, width
				- getPaddingRight(), centerItemTop, paint);
		canvas.drawLine(getPaddingLeft(), centerItemBottom, width
				- getPaddingRight(), centerItemBottom, paint);
		coverPaint.setShader(shader);
	...
	public void initSelectionPosi(int posi)
	{
		cacheNowItem = posi;
	}
	...
	private void checkStateAndPosition()
	{
		// 上拉超出
		if (!isCircle
				&& scrollY < -(getRealHeight() - (showSize + 1) / 2
						* itemHeight))
		{
			mScroller.startScroll(0, (int) scrollY, 0, (showSize + 1) / 2
					* itemHeight - getRealHeight() - (int) scrollY, 400);
			// mScroller.springBack(0,(int)scrollY,0,0,minScrollY,maxScrollY);
		} else if (!isCircle && scrollY > (showSize - 1) / 2 * itemHeight)
		{ // 下拉超出
			mScroller.startScroll(0, (int) scrollY, 0, (showSize - 1) / 2
					* itemHeight - (int) scrollY, 400);
			// mScroller.springBack(0,(int)scrollY,0,0,minScrollY,maxScrollY);
		} else
		{
			long endTime = System.currentTimeMillis();
			// 超出滑动时间或者不足滑动距离
			if (endTime - downTime > 250
					|| Math.abs(lastY - downY) < itemHeight / 2)
			{
				int dy = (int) scrollY % itemHeight; // 不足一个Item高度的部分
				if (Math.abs(dy) > itemHeight / 2)
				{ // 如果偏移大于item的一半，
					if (scrollY < 0)
					{
						mScroller.startScroll(0, (int) scrollY, 0, -itemHeight
								- dy);
					} else
					{
						mScroller.startScroll(0, (int) scrollY, 0, itemHeight
			...
			} else
			{
				...
				int finalY = (int) ((scrollY + rate * (lastY - downY)
						/ (endTime - downTime)))
				...
				mScroller.startScroll(0, (int) scrollY, 0,
						(int) (finalY - scrollY), 400);
			}
		}
	}
